...
for (i in 1:elements_page) {
  ...
  # results_new <- list.stack(results_new)

  results <- rbind(results,results_new)
  
  'venues_name[[i]] <- venues[[i]][["name"]]
  venues_city[[i]] <- venues[[i]][["city"]][["name"]]
  venues_postalCode[[i]] <- venues[[i]][["postalCode"]]
  venues_address[[i]] <- venues[[i]][["address"]][["line1"]]
  venues_url[[i]] <- venues[[i]][["url"]]
  venues_longitude[[i]] <- venues[[i]][["location"]][["longitude"]]
  venues_latitude[[i]] <- if (!is.null(venues[[i]][["location"]])) {venues[[i]][["location"]][["latitude"]]} else {NULL}'
  ...
  venues_list <- list.stack(api_content$Search)
  
  results <- rbind(results,venues_list)'
}
